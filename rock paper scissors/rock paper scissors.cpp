	...
	const choice plchoice () throw ()
	{
		...
		while (true)
		{
			if (backend::data::username)
				cout<<"Enter rock (r), paper (p), or scissors (s) to play a round, statistics (st) for win/loss/tie statistics, history (h) for rounds history, comphist (c) for\ncomplex rounds history, clear (cl) to clear records and start again, save (sv) to save your data to disk, reload (rl) to reload the last save to disk, change (cp) to update your password, or exit (e) or quit (q) to exit:\n";
			else
			...
			else if (backend::data::username)
			{
				...
				else if (!(stricmp(ans, "reload") && stricmp(ans, "rl")))
				{
					string tmp;
					cout << "About to reload data from disk.\nThis will erase ALL unsaved data, including unsaved password changes.\n";
					while (true)
					{
						cin.clear();
						...
						if (tmp == "y" || tmp == "Y")
						{
							cout << "Reloading data from disk...\n";
							break;
						}
						else if (tmp == "n" || tmp == "N" || tmp=="")
						{
							cout << "Abort.\n\n";
							return nochoice;
						}
					...
					if (res == failure)
					{
						cout << "Unable to load data.\nAttempting compatibility mode load...\n";
						res = compatload();
						if (res == failure)
						{
							cout << "Failed.\n\n";
							return nochoice; // Technically, this is a failure condition, but oh well.
						}
						else if (res == bad)
						{
							cout << "Incorrect password.\nChange the in-memory password (change) to reflect the saved password.\n\n";
							return nochoice; // See above
						}
					}
					else if (res == bad)
					{
					...
					disphistto(cout << "Data reloaded from disk.\nCurrent statistics:\n");
					return nochoice;
				}
		...
		else
		{
			...
			while (1)
			{
				...
				if (!stricmp(mode.c_str(), "database") || !stricmp(mode.c_str(), "d"))
				{
					...
					backend::data::username = new char[user.size() + 1];
					strcpy_s(backend::data::username, user.size() + 1, user.c_str());
					const bool load((bool)ifstream((user + ".db").c_str()));
					backend::data::database.open((user + ".db").c_str(), ios::in | ios::out);
					cout << "Please enter your password.\n";
					cin.clear();
					cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
